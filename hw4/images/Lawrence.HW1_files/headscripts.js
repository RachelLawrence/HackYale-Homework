...
function loadXMLDoc(url)
{
	...
	if (window.XMLHttpRequest)
	{
		...
		updateReq.open("GET", url + ch+"auto=true&unq=" + new Date().getTime() + "-" +  Math.random(), true);
		updateReq.send(null);
	}
...
//YALE CHANGES START:
//Following methods have been added in for yale-publish-site at portal
var xmlHttp;

function publishSite(url)
{
	if(!confirm("Students cannot see course sites until they are published.  Do you want to publish this site now?"))
		return;
	
	var index = url.indexOf('site');
	url = url.substring(0,index) + "yaleEvent/" + url.substring(index) + "/?yalePublishSite=yes";
	
	// branch for native XMLHttpRequest object
	if (window.XMLHttpRequest)
	{
		xmlHttp = new XMLHttpRequest();
		xmlHttp.onreadystatechange = processSitePublished;
...
function processSitePublished()
{
	try
	{
		if (xmlHttp.readyState == 4)
		{
			...
			{
				alert("Error occurred while publishing the site");
				return;
			}
			
			var tag = document.getElementById("yu_site_publish");
...
//Following function are for userAction.java & chef_user_view.vm page
function updateMyPhotoOption(isChecked, url,netId){
	var value="no";
	if (isChecked)
		value="yes";
	
	if(!confirm("Are you sure?")){
		var checkBox = document.getElementById("showPhotoPublic");
		checkBox.checked = !isChecked;
		return;
		}
		
	var index = url.indexOf('site');
	url = url.substring(0,index) + "yaleEvent/" + url.substring(index) + "/?yaleShowMyPhoto=" + value +"&photoNetId=" +netId;
	
	// branch for native XMLHttpRequest object
	if (window.XMLHttpRequest)
	{
		xmlHttp = new XMLHttpRequest();
		xmlHttp.onreadystatechange = processShowMyPhoto;
		xmlHttp.open("GET", url, true);
		xmlHttp.send(null);
	}
...
function processShowMyPhoto()
{	
	try
	{
		if (xmlHttp.readyState == 4)
		{
			var tag = document.getElementById("photoUpdatingResponseMsgs");
			
			result = xmlHttp.responseText;						
			if(result.indexOf("Error")!= -1)
			{
				result = "Error occurred while updating";
				tag.className='alertMessage';//change css class
			}
...
//used for avoid double-click crash purpose
function switchButtons(enabledBtnId, disabledBtnId){
  var enabledBtnTag = document.getElementById(enabledBtnId);
  var disabledBtnTag = document.getElementById(disabledBtnId);
  if (enabledBtnTag && disabledBtnTag){		      
    enabledBtnTag.style.display='none';
    disabledBtnTag.style.display="";
    document.body.style.cursor='wait';//IE browser
  }		    		
}

function hideFirst(){
		try{
		   var helpImg = $('div.portletTitle .action a');
		   $(helpImg).css('visibility', 'hidden');
		}
		catch(error){}
	}

	function moveHelpLinks(){
		  try {
			  var helpImg = $('div.portletTitle .action a');
			  var newLink = $("<a>Help</a>");
			  
			  var newLi = $("<li></li>");
			  
			  var newSpan = $("<span></span>");
			  $(newLink).attr('href', $(helpImg).attr('href'));
			  $(newLink).attr('onclick', $(helpImg).attr('onclick'));
			  $(newLink).attr('target', $(helpImg).attr('target'));
			  $(newLink).attr('title', $(helpImg).attr('title'));
			  $(newLi).append(newSpan);
			  $(newSpan).append(newLink);
			  $(".portletTitle >.action:first").append(newSpan);
			  $(newSpan).css('float', 'left');
			  $(newSpan).css('padding-left', '20px');
			  var offset = $('.portletTitle > .title h2').offset();
			//  alert("top="+offset.top+" left = "+offset.left+" height="+offset.height);
			  $(newSpan).css('margin-top', '5px');
		}
		catch(error){}
	...
	function hideHelpLinkInMyworkspace(){
		var title = $('title').text();
		
		if (title.indexOf('My Workspace : Home')>=0){
			$('div.portletTitle .action').each(function(index, value){
				
				var text = $(value).text();
				if ( trim0(text.toLowerCase()) =="help"){
	...
	function hideSyllabus(){
		...
		var title = $('title').text();
		if (title.indexOf('Gateway : Syllabus Browser')>=0){
			$('iframe').each(function(index, value){
				var h3 = $(this).contents().find('h3');
				var text=$(h3).text();
				if (trim0(text)=="Syllabus Browser"){
					$(h3).removeClass('hide').addClass('show');
				}
			});
		}
	}
	...
	function switchImg(){
		$("iframe:first-child").contents().find("img[src='classes_welcome_img.jpg']").attr('src', 'http://localhost:8080/classes_welcome_img.jpg' );
