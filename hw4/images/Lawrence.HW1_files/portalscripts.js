...
var dhtml_more_tabs = function() {
	...
	jQuery('#selectNav').width("600px");           // set its width to fix an IE6 bug //IN FIX width
...
function show_timeout_alert(min) {
	...
	if (jQuery("#timeout_alert_body").get(0)) {
		...
		jQuery('#timeout_alert_body').css({ 'position': 'fixed', 'z-index':'10001', 'background-color':'#FAD496',  'padding': '10px', 'border':'1px solid #F19400'});
		jQuery('#timeout_alert_body').css("top", "45%");//Math.max(0, ((jQuery(window).height() - jQuery('#timeout_alert_body').outerHeight()) / 2) +  jQuery(window).scrollTop()) + "px");
 		jQuery('#timeout_alert_body').css("left", "43%");//Math.max(0, ((jQuery(window).width() - jQuery('#timeout_alert_body').outerWidth()) / 2) + jQuery(window).scrollLeft()) + "px");        
 		jQuery('#alert_button').css({'color': '#1F4E91', 'margin-left':'34px', 'z-index':'10001'});
 		jQuery('#alert_button').css('border-style', 'none');
 		jQuery('#alert_button').css('cursor', 'pointer');
 		jQuery('#alert_button').css('padding', '10px');
 		jQuery('#alert_button').css('margin-top', '10px');
	} else {
